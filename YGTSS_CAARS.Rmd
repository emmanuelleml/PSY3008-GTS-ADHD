...
CAARS = data.frame(id = caars$PatientCode, tps_de_mesure = as.character(caars$tblCondition.Nom), A = caars$CAARSA, B = caars$CAARSB, C = caars$CAARSC, D = caars$CAARSD, E = caars$CAARSE, score = caars$Total)
CAARS
sjmisc::frq(CAARS$tps_de_mesure)


CAARS <- CAARS[!(CAARS$tps_de_mesure== "TOC controle"),]
...
CAARS_wide
CAARS
# Nouveau fichier avec les bons tps de mesure
write_xlsx(CAARS,"Excel/MarcCAARSwide_Emmanuelle.xlsx")


# DF CAARS officiel
caars_2 <- read_excel("Excel/MarcCAARSwide_Emmanuelle.xlsx")
caars_2
CAARS_2 <- data.frame(id = caars_2$id, tps_de_mesure = caars_2$tps_de_mesure, CAARSA = as.numeric(caars_2$A), CAARSB = as.numeric(caars_2$B), CAARSC = as.numeric(caars_2$C), CAARSD = as.numeric(caars_2$D), CAARSE = as.numeric(caars_2$E), CAARS = as.numeric(caars_2$score))
CAARS_2
CAARS_wide_2 <- reshape(CAARS_2, idvar = "id", timevar = "tps_de_mesure", direction = "wide")
```
...
BAI <-  data.frame(id = bai$PatientCode, tps_de_mesure = bai$Condition, BAI = as.numeric(bai$IABTot))
...
BAI <- BAI[!(BAI$tps_de_mesure == "Exclusion" | BAI$tps_de_mesure=="abandon" | BAI$tps_de_mesure== "desistement" | BAI$tps_de_mesure== "TOC_controle" | BAI$tps_de_mesure == "Post_T1" | BAI$tps_de_mesure == "Post_T2" | BAI$tps_de_mesure == "Post" | BAI$tps_de_mesure == "Post-RV" | BAI$tps_de_mesure == "post_6_mois" | BAI$tps_de_mesure == "post_1_an" | BAI$tps_de_mesure =="Bloc_0" | BAI$tps_de_mesure=="MI" | BAI$tps_de_mesure== "Mid"| BAI$tps_de_mesure== "post"),]
...
# 1689 et 1855 ont juste le tps d emesure controle, donc --> Pre
subset(BAI, tps_de_mesure == "controle")
BAI['tps_de_mesure'][BAI['tps_de_mesure'] == "controle"] <- "Pre"

# ont tous aussi un Pre. donc --> Pre_T2
...
# 2 id ont juste un Pre T2
subset(BAI_wide_2, !is.na(BAI_wide_2$BAI.Pre_T2) & is.na(BAI_wide_2$BAI.Pre) & is.na(BAI_wide_2$BAI.Pre_T1))
BAI$tps_de_mesure <- ifelse((BAI$id == "2500" | BAI$id == "3185"), "Pre", BAI$tps_de_mesure)

BAI_wide_3 = reshape(BAI, idvar = "id", timevar = "tps_de_mesure", direction = "wide")
BAI_wide_3

# Ceux qui ont Pre et T1
subset(BAI_wide_3, !is.na(BAI_wide_3$BAI.Pre_T1) & !is.na(BAI_wide_3$BAI.Pre) & is.na(BAI_wide_3$BAI.Pre_T2))
BAI_wide_3[BAI_wide_3$id=="1945",] <- c("1945", 15, 11, NA)
BAI_wide_3[BAI_wide_3$id=="1945",]
BAI_wide_3[BAI_wide_3$id=="2300",] <- c("2300", 14, 17, NA)
BAI_wide_3[BAI_wide_3$id=="2300",]

# Ceux qui ont Pre et T2
subset(BAI_wide_3, is.na(BAI_wide_3$BAI.Pre_T1) & !is.na(BAI_wide_3$BAI.Pre) & !is.na(BAI_wide_3$BAI.Pre_T2))
BAI_wide_3[BAI_wide_3$id=="1532",] <- c("1532", 13, 6, NA)
BAI_wide_3[BAI_wide_3$id=="2137",] <- c("2137", 3, 7, NA)
BAI_wide_3[BAI_wide_3$id=="2145",] <- c("2145", 12, 19, NA)
BAI_wide_3[BAI_wide_3$id=="2146",] <- c("2146", 6, 11, NA)
# 256 id ont juste un T1
subset(BAI_wide_3, !is.na(BAI_wide_3$BAI.Pre_T1) & is.na(BAI_wide_3$BAI.Pre) & is.na(BAI_wide_3$BAI.Pre_T2))
...
bdi
BDI <-  data.frame(id = bdi$PatientCode, tps_de_mesure = bdi$tblCondition.Nom, bdi = as.numeric(bdi$BDITot))
...
# 1689 et 1855 ont juste controle, donc --> Pre
subset(BDI, tps_de_mesure == "controle")
...
# 4 id ont juste un Pre T2
subset(BDI_wide_2, !is.na(BDI_wide_2$bdi.Pre_T2) & is.na(BDI_wide_2$bdi.Pre) & is.na(BDI_wide_2$bdi.Pre_T1))
BDI$tps_de_mesure <- ifelse((BDI$id == "2603" | BDI$id == "3170" | BDI$id == "3181" | BDI$id == "3185"), "Pre", BDI$tps_de_mesure)
...
BDI_wide_3[BDI_wide_3$id=="1945",] <- c("1945", NA, 18, 28)
BDI_wide_3[BDI_wide_3$id=="1945",]
BDI_wide_3[BDI_wide_3$id=="2300",] <- c("2300", NA, 29, 21)
BDI_wide_3[BDI_wide_3$id=="2300",]
BDI_wide_3[BDI_wide_3$id=="8000",] <- c("8000", NA, 13, 28)
BDI_wide_3[BDI_wide_3$id=="8000",]

# 243 id ont juste un T1
subset(BDI_wide_3, !is.na(BDI_wide_3$bdi.Pre_T1) & is.na(BDI_wide_3$bdi.Pre) & is.na(BDI_wide_3$bdi.Pre_T2))
...
```{r}
df_CAARS_YGTSS <- merge(CAARS_wide_2, YGTSS_wide, by.x = "id", by.y = "id", all = TRUE)
...
# Retrait des participants qui n'ont pas fait le CAARS (mais qui ont été ajoutés au df pcq ont fait le YGTSS)
```{r}
df_all__ <- df_w_HD[complete.cases(df_w_HD$CAARS.Pre), ]
df_all_ <- subset(df_all__, !is.na(BAI_pre) | !is.na(BAI_T1)| !is.na(BAI_T2))
df_allz <- subset(df_all__, !is.na(BDI_pre) | !is.na(BDI_T1)| !is.na(BDI_T2))

subset(df_allz, categorie == "HD")
df_all <- subset(df_allz, id != 1663 & id != 1667 & id != 1670 & id != 1672 & id != 1673 & id != 1696 & id != 1810 & id != 2023 & id != 2026 & id != 2028 & id != 2057 & id != 2063 & id != 2065 & id != 2073 & id != 2076)
df_all
```
...
df_SGT <- df_all[complete.cases(df_all$YGTSS.Pre|df_all$YGTSS.Pre_T1|df_all$YGTSS.Pre_T2), ]
#df_SGT # 91 participants SGT+ ou SGT -
median(df_SGT$CAARS.Pre, na.rm = TRUE) #33

# Médiane CAARS des contrôles
df_ctrl_all <- df_all[!complete.cases(df_all$YGTSS.Pre|df_all$YGTSS.Pre_T1|df_all$YGTSS.Pre_T2), ]
#df_ctrl_all # 165 participants ctrl ou TDAH-
median(df_ctrl_all$CAARS.Pre) #30

df_all$groupe <- ifelse(is.na(df_all$YGTSS.Pre) & is.na(df_all$YGTSS.Pre_T1) & is.na(df_all$YGTSS.Pre_T2) & df_all$CAARS.Pre < 30, "Contrôle-", 0)
df_all$groupe <- ifelse(is.na(df_all$YGTSS.Pre) & is.na(df_all$YGTSS.Pre_T1) & is.na(df_all$YGTSS.Pre_T2) & df_all$CAARS.Pre >= 30, "Contrôle+", df_all$groupe)
df_all$groupe <- ifelse(!(df_all$groupe == "Contrôle-") & !(df_all$groupe == "Contrôle+") & df_all$CAARS.Pre < 33, "SGT-",df_all$groupe)
df_all$groupe <- ifelse(!(df_all$groupe == "Contrôle-") & !(df_all$groupe == "Contrôle+") & df_all$CAARS.Pre >= 33, "SGT+",df_all$groupe)

df_SGT$groupe <- ifelse(df_SGT$CAARS.Pre < 32, "SGT-", 0)
df_SGT$groupe <- ifelse(df_SGT$CAARS.Pre >= 32, "SGT+",df_SGT$groupe)
...
sd_T2_BAI = sd(df_SGT$BAI_T2, na.rm = TRUE)
...
jmv::descriptives(df_all, vars(delta_BAI_pre), skew = T, kurt = T) # NOT OK

subset(df_all, abs(delta_BAI_pre) >= IC90_BAI)
...
# BDI
subset(df_all, !is.na(BDI_pre) & !is.na(BDI_T2)) # 2137, 2145 & 2146
subset(df_all, is.na(BDI_pre) & is.na(BDI_T1) & !is.na(BDI_T2))
subset(df_all, is.na(BDI_pre) & !is.na(BDI_T1) & is.na(BDI_T2))

df_all$BDI_T1 <- ifelse(df_all$id == 2137 | df_all$id == 2145 | df_all$id == 2146, df_all$BDI_pre, df_all$BDI_T1)
df_all$BDI_pre <- ifelse(df_all$id == 2137 | df_all$id == 2145 | df_all$id == 2146, NA, df_all$BDI_pre)



sd_T1_BDI = sd(df_all$BDI_T1, na.rm = TRUE)
SEM_T1_BDI = sd_T1_BDI*sqrt(1-0.75)
sd_T2_BDI = sd(df_SGT$BDI_T2, na.rm = TRUE)
...
subset(df_all, abs(delta_BDI_pre) >= IC90_BDI) # juste id 3112

df_all$BDI_pre_mean <- ifelse(is.na(df_all$BDI_pre),rowMeans(df_all[,c('BDI_T1', 'BDI_T2')], na.rm=TRUE), df_all$BDI_pre)
df_all$BDI_pre_mean <- ifelse(df_all$id == 3112, df_all$BDI_T2, df_all$BDI_pre_mean)
#df_all$BDI_pre_mean
...
```{r}
# Parmi ceux qui ont fait le YGTSS (donc ceux du df_SGT), est-ce que ceux qui ont pas fait Pre ont fait Pre 6 mois et Pre 1 an ? ou y'en a qui ont fait les 3 ou juste T1 ou T2?
subset(df_SGT, is.na(YGTSS.Pre))# 2060 a fait Pre T2 mais pas T1

# Est-ce que ceux qui ont fait le Pre ont aussi pas fait le prré t1 et Pre t2
subset(df_SGT, !is.na(YGTSS.Pre)) # --> OUI. Donc pour tous ceux qui ont fait Pre, on prend leur score Pre
```
...
# Différence T1 T2. Ajout dans le df complet et dans le df juste SGT
...
subset(df_all, abs(delta_YGTSS.Pre) >= IC90) # id 2034 & 2079
...
df_all$YGTSS.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2034 | df_all$id == 2079, df_all$YGTSS.Pre_T2, df_all$YGTSS.Pre_mean)
```
...
subset(df_all, abs(delta_nbr_moteur_Pre) >= IC90_nbr_moteur) # id 1642, 2023 & 2040
...
df_all$nbr_moteur.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 1642 | df_all$id == 2023 | df_all$id == 2040, df_all$nbr_moteur.Pre_T2, df_all$nbr_moteur.Pre_mean)
```
...
#id 2060 qui a juste T2
df_all$nbr_phonique.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2029 | df_all$id == 2031 | df_all$id == 2042 | df_all$id == 2058 | df_all$id == 2079, df_all$nbr_phonique.Pre_T2, df_all$nbr_phonique.Pre_mean)
```
...
subset(df_all, abs(delta_frq_moteur_Pre) >= IC90_frq_moteur) # id 2026
...
#id 2060 qui a juste T2
df_all$frq_moteur.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2026, df_all$frq_moteur.Pre_T2, df_all$frq_moteur.Pre_mean)
```
...
#id 2060 qui a juste T2
df_all$frq_phonique.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2042 | df_all$id == 2058 | df_all$id == 2079, df_all$frq_phonique.Pre_T2, df_all$frq_phonique.Pre_mean)
```
## Int_moteur
```{r}
...
subset(df_all, abs(delta_int_moteur_Pre) >= IC90_int_moteur) # 8 id
...
df_all$int_moteur.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 1627 | df_all$id == 1642 | df_all$id == 2040 | df_all$id == 2055 | 
                                       df_all$id == 2065 | df_all$id == 2069 | df_all$id == 2074 | df_all$id == 2076, df_all$int_moteur.Pre_T2, df_all$int_moteur.Pre_mean)
```
## Int_phonique
```{r}
...
#id 2060 qui a juste T2
df_all$int_phonique.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2042 | df_all$id == 2058, df_all$int_phonique.Pre_T2, df_all$int_phonique.Pre_mean)
```
## Comp_moteur
```{r}
...
df_all$comp_moteur.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 1488 | df_all$id == 2029 | df_all$id == 2069, df_all$comp_moteur.Pre_T2, df_all$comp_moteur.Pre_mean)
```
...
#id 2060 qui a juste T2
df_all$comp_phonique.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2038 | df_all$id == 2069, df_all$comp_phonique.Pre_T2, df_all$comp_phonique.Pre_mean)
```
## Inter_moteur
```{r}
...
df_all$inter_moteur.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2061 | df_all$id == 2065 | df_all$id == 2074, df_all$inter_moteur.Pre_T2, df_all$inter_moteur.Pre_mean)
```
...
#id 2060 qui a juste T2
df_all$inter_phonique.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2061, df_all$inter_phonique.Pre_T2, df_all$inter_phonique.Pre_mean)
```
...
df_all$deter_moteur.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 1627 | df_all$id == 1642 | df_all$id == 1821 | df_all$id == 1863 | df_all$id == 2029
                                       | df_all$id == 2034 | df_all$id == 2055 | df_all$id == 2073 | df_all$id == 2076 | df_all$id == 2084, df_all$deter_moteur.Pre_T2, df_all$deter_moteur.Pre_mean)
```
...
#id 2060 qui a juste T2
df_all$deter_phonique.Pre_mean <- ifelse(df_all$id == 2060 | df_all$id == 2034 | df_all$id == 2079, df_all$deter_phonique.Pre_T2, df_all$deter_phonique.Pre_mean)
```
...
tab_complete_2 <- tab_complete[complete.cases(tab_complete$CAARS.Pre),]
tab_complete_2
```
...
```{r}
tab <- readxl::read_excel("Excel/data_stats.xlsx")
tab
agesexe <- readxl::read_excel("Excel/age et sexe.xlsx")
age_sexe <- data.frame(id = agesexe$PatientCode, date_naissance = agesexe$DateDeNaissance, sexe = agesexe$Sexe)
age_sexe
tab_complete_3 <- merge(tab, age_sexe, by.x = "id", by.y = "id", all = TRUE)
tab_complete_3
tab_complete_4 <- tab_complete_3[complete.cases(tab_complete_3$CAARS.Pre),]
tab_complete_4

write_xlsx(x = tab_complete_4, path = "Excel/age_sexe_265.xlsx", col_names = TRUE)
```
